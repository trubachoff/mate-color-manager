...
typedef enum {
	MCM_CALIBRATE_ARGYLL_STATE_IDLE,
	MCM_CALIBRATE_ARGYLL_STATE_WAITING_FOR_STDIN,
	MCM_CALIBRATE_ARGYLL_STATE_WAITING_FOR_LOOP,
	MCM_CALIBRATE_ARGYLL_STATE_RUNNING,
	MCM_CALIBRATE_ARGYLL_STATE_LAST
} McmCalibrateArgyllState;

/**
 * McmCalibrateArgyllPrivate:
 *
 * Private #McmCalibrateArgyll data
 **/
struct _McmCalibrateArgyllPrivate
{
	guint				 display;
	GMainLoop			*loop;
	GtkWidget			*terminal;
...
G_DEFINE_TYPE (McmCalibrateArgyll, mcm_calibrate_argyll, MCM_TYPE_CALIBRATE)

/**
 * mcm_calibrate_argyll_get_quality_arg:
 **/
static const gchar *
mcm_calibrate_argyll_get_quality_arg (McmCalibrateArgyll *calibrate_argyll)
{
	...
	if (reference_kind == MCM_CALIBRATE_REFERENCE_KIND_COLOR_CHECKER ||
	    reference_kind == MCM_CALIBRATE_REFERENCE_KIND_QPCARD_201)
		return "-ql";

	/* get the default precision */
	if (precision == MCM_CALIBRATE_PRECISION_SHORT)
		return "-ql";
	if (precision == MCM_CALIBRATE_PRECISION_NORMAL)
		return "-qm";
	if (precision == MCM_CALIBRATE_PRECISION_LONG)
		return "-qh";
	return "-qm";
}

/**
 * mcm_calibrate_argyll_display_get_patches:
 **/
static guint
mcm_calibrate_argyll_display_get_patches (McmCalibrateArgyll *calibrate_argyll)
{
	guint patches = 250;
	McmCalibratePrecision precision;
	...
	if (precision == MCM_CALIBRATE_PRECISION_SHORT)
		patches = 100;
	else if (precision == MCM_CALIBRATE_PRECISION_NORMAL)
		patches = 250;
	else if (precision == MCM_CALIBRATE_PRECISION_LONG)
		patches = 500;
	return patches;
}

/**
 * mcm_calibrate_argyll_printer_get_patches:
 **/
static guint
mcm_calibrate_argyll_printer_get_patches (McmCalibrateArgyll *calibrate_argyll)
{
	guint patches = 180;
	...
	if (colorimeter_kind == MCM_COLORIMETER_KIND_COLOR_MUNKI ||
	    colorimeter_kind == MCM_COLORIMETER_KIND_SPECTRO_SCAN) {
		patches *= 2;
	}
 ...
 **/
static const gchar *
mcm_calibrate_argyll_get_colorimeter_image_attach (McmCalibrateArgyll *calibrate_argyll)
{
	McmColorimeterKind colorimeter_kind;
...
/**
 * mcm_calibrate_argyll_get_colorimeter_image_calibrate:
 **/
static const gchar *
mcm_calibrate_argyll_get_colorimeter_image_calibrate (McmCalibrateArgyll *calibrate_argyll)
{
	McmColorimeterKind colorimeter_kind;
...
/**
 * mcm_calibrate_argyll_get_colorimeter_image_screen:
 **/
static const gchar *
mcm_calibrate_argyll_get_colorimeter_image_screen (McmCalibrateArgyll *calibrate_argyll)
{
	McmColorimeterKind colorimeter_kind;
...
/**
 * mcm_calibrate_argyll_get_display:
 **/
static guint
mcm_calibrate_argyll_get_display (const gchar *output_name, GError **error)
{
	gboolean ret;
	gchar *data = NULL;
	gchar **split = NULL;
...
/**
 * mcm_calibrate_argyll_get_display_kind:
 **/
static gchar
mcm_calibrate_argyll_get_display_kind (McmCalibrateArgyll *calibrate_argyll)
{
	...
	if (device_kind == MCM_CALIBRATE_DEVICE_KIND_LCD)
		return 'l';
	if (device_kind == MCM_CALIBRATE_DEVICE_KIND_CRT)
		return 'c';
	if (device_kind == MCM_CALIBRATE_DEVICE_KIND_PROJECTOR)
		return 'p';
	return '\0';
}

/**
 * mcm_calibrate_argyll_debug_argv:
 **/
static void
mcm_calibrate_argyll_debug_argv (const gchar *program, gchar **argv)
{
	gchar *join;
...
/**
 * mcm_calibrate_argyll_get_tool_filename:
 **/
static gchar *
mcm_calibrate_argyll_get_tool_filename (const gchar *command, GError **error)
{
	gboolean ret;
	gchar *filename;
...
/**
 * mcm_calibrate_argyll_fork_command:
 **/
static gboolean
mcm_calibrate_argyll_fork_command (McmCalibrateArgyll *calibrate_argyll, gchar **argv, GError **error)
{
	...
	/* clear */
	priv->state = MCM_CALIBRATE_ARGYLL_STATE_IDLE;
...
/**
 * mcm_calibrate_argyll_display_neutralise:
 **/
static gboolean
mcm_calibrate_argyll_display_neutralise (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar kind;
	...
	command = mcm_calibrate_argyll_get_tool_filename ("dispcal", error);
	if (command == NULL) {
		ret = FALSE;
		goto out;
	}
	...
	priv->display = mcm_calibrate_argyll_get_display (output_name, error);
	...
	/* get l-cd or c-rt */
	kind = mcm_calibrate_argyll_get_display_kind (calibrate_argyll);
...
/**
 * mcm_calibrate_argyll_display_read_chart:
 **/
static gboolean
mcm_calibrate_argyll_display_read_chart (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar *command = NULL;
	...
	/* get correct name of the command */
	command = mcm_calibrate_argyll_get_tool_filename ("chartread", error);
	if (command == NULL) {
		ret = FALSE;
		goto out;
	}
...
/**
 * mcm_calibrate_argyll_display_generate_patches:
 **/
static gboolean
mcm_calibrate_argyll_display_generate_patches (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar *command = NULL;
	...
	/* get correct name of the command */
	command = mcm_calibrate_argyll_get_tool_filename ("targen", error);
	if (command == NULL) {
		ret = FALSE;
		goto out;
	}
	...
	if (device_kind == MCM_DEVICE_KIND_DISPLAY)
		g_ptr_array_add (array, g_strdup_printf ("-f%i", mcm_calibrate_argyll_display_get_patches (calibrate_argyll)));
	else if (device_kind == MCM_DEVICE_KIND_PRINTER)
		g_ptr_array_add (array, g_strdup_printf ("-f%i", mcm_calibrate_argyll_printer_get_patches (calibrate_argyll)));
...
/**
 * mcm_calibrate_argyll_display_draw_and_measure:
 **/
static gboolean
mcm_calibrate_argyll_display_draw_and_measure (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar kind;
	...
	/* get correct name of the command */
	command = mcm_calibrate_argyll_get_tool_filename ("dispread", error);
	if (command == NULL) {
		ret = FALSE;
		goto out;
	}
...
/**
 * mcm_calibrate_argyll_display_generate_profile:
 **/
static gboolean
mcm_calibrate_argyll_display_generate_profile (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar **argv = NULL;
	...
	model = mcm_calibrate_get_model_fallback (MCM_CALIBRATE (calibrate_argyll));
	manufacturer = mcm_calibrate_get_manufacturer_fallback (MCM_CALIBRATE (calibrate_argyll));
	description = mcm_calibrate_get_description_fallback (MCM_CALIBRATE (calibrate_argyll));

	/* get correct name of the command */
	command = mcm_calibrate_argyll_get_tool_filename ("colprof", error);
	if (command == NULL) {
		ret = FALSE;
		goto out;
	}
	...
	g_ptr_array_add (array, g_strdup (mcm_calibrate_argyll_get_quality_arg (calibrate_argyll)));
...
/**
 * mcm_calibrate_argyll_reference_kind_to_filename:
 **/
static const gchar *
mcm_calibrate_argyll_reference_kind_to_filename (McmCalibrateReferenceKind kind)
{
...
/**
 * mcm_calibrate_argyll_device_copy:
 **/
static gboolean
mcm_calibrate_argyll_device_copy (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret;
	gchar *device = NULL;
	...
	g_object_get (calibrate_argyll,
		      ...
		      NULL);
	working_path = mcm_calibrate_get_working_path (MCM_CALIBRATE (calibrate_argyll));
	...
	filename_tmp = mcm_calibrate_argyll_reference_kind_to_filename (reference_kind);
	...
	/* do the copy */
	ret = mcm_utils_mkdir_and_copy (file_cht, dest_cht, error);
	if (!ret)
		goto out;
	ret = mcm_utils_mkdir_and_copy (file_source, dest_source, error);
	if (!ret)
		goto out;
	ret = mcm_utils_mkdir_and_copy (file_reference, dest_reference, error);
	if (!ret)
		goto out;
out:
	g_free (basename);
...
/**
 * mcm_calibrate_argyll_device_measure:
 **/
static gboolean
mcm_calibrate_argyll_device_measure (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar **argv = NULL;
	...
	/* get correct name of the command */
	command = mcm_calibrate_argyll_get_tool_filename ("scanin", error);
	if (command == NULL) {
		ret = FALSE;
		goto out;
	}
...
/**
 * mcm_calibrate_argyll_device_generate_profile:
 **/
static gboolean
mcm_calibrate_argyll_device_generate_profile (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar **argv = NULL;
	...
	manufacturer = mcm_calibrate_get_manufacturer_fallback (MCM_CALIBRATE (calibrate_argyll));
	description = mcm_calibrate_get_description_fallback (MCM_CALIBRATE (calibrate_argyll));
	device = mcm_calibrate_get_device_fallback (MCM_CALIBRATE (calibrate_argyll));
...
/**
 * mcm_calibrate_argyll_set_filename_result:
 **/
static gboolean
mcm_calibrate_argyll_set_filename_result (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gchar *filename = NULL;
	gboolean ret = TRUE;
	gchar *basename = NULL;
...
/**
 * mcm_calibrate_argyll_remove_temp_files:
 **/
static gboolean
mcm_calibrate_argyll_remove_temp_files (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gchar *filename_tmp;
	guint i;
	gboolean ret;
...
/**
 * mcm_calibrate_argyll_display:
 **/
static gboolean
mcm_calibrate_argyll_display (McmCalibrate *calibrate, GtkWindow *window, GError **error)
{
	...
	/* step 1 */
	ret = mcm_calibrate_argyll_display_neutralise (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 2 */
	ret = mcm_calibrate_argyll_display_generate_patches (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 3 */
	ret = mcm_calibrate_argyll_display_draw_and_measure (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 4 */
	ret = mcm_calibrate_argyll_display_generate_profile (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 5 */
	ret = mcm_calibrate_argyll_remove_temp_files (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 6 */
	ret = mcm_calibrate_argyll_set_filename_result (calibrate_argyll, error);
	if (!ret)
		goto out;
out:
	return ret;
...
/**
 * mcm_calibrate_argyll_spotread_read_chart:
 **/
static gboolean
mcm_calibrate_argyll_spotread_read_chart (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar *command = NULL;
...
/**
 * mcm_calibrate_argyll_spotread:
 **/
static gboolean
mcm_calibrate_argyll_spotread (McmCalibrate *calibrate, GtkWindow *window, GError **error)
{
	McmCalibrateArgyll *calibrate_argyll = MCM_CALIBRATE_ARGYLL(calibrate);
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gboolean ret;
...
/**
 * mcm_calibrate_argyll_get_colorimeter_target:
 **/
static const gchar *
mcm_calibrate_argyll_get_colorimeter_target (McmCalibrateArgyll *calibrate_argyll)
{
	McmColorimeterKind colorimeter_kind;
...
/**
 * mcm_calibrate_argyll_display_generate_targets:
 **/
static gboolean
mcm_calibrate_argyll_display_generate_targets (McmCalibrateArgyll *calibrate_argyll, gdouble width, gdouble height, GError **error)
{
	gboolean ret = TRUE;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gchar *command = NULL;
	...
	/* get correct name of the command */
	command = mcm_calibrate_argyll_get_tool_filename ("printtarg", error);
	if (command == NULL) {
		ret = FALSE;
		goto out;
	}
	...
	/* target instrument */
	g_ptr_array_add (array, g_strdup_printf ("-i%s", mcm_calibrate_argyll_get_colorimeter_target (calibrate_argyll)));
...
/**
 * mcm_calibrate_argyll_render_cb:
 **/
static GPtrArray *
mcm_calibrate_argyll_render_cb (McmPrint *print, GtkPageSetup *page_setup, McmCalibrate *calibrate, GError **error)
{
	gboolean ret = TRUE;
	GPtrArray *array = NULL;
	...
	g_object_get (calibrate,
		      "basename", &basename,
		      NULL);
	working_path = mcm_calibrate_get_working_path (calibrate);
	...
	ret = mcm_calibrate_argyll_display_generate_targets (MCM_CALIBRATE_ARGYLL (calibrate), width, height, error);
	if (!ret)
		goto out;
...
/**
 * mcm_calibrate_argyll_set_device_from_ti2:
 **/
static gboolean
mcm_calibrate_argyll_set_device_from_ti2 (McmCalibrate *calibrate, const gchar *filename, GError **error)
{
	gboolean ret;
	gchar *contents = NULL;
	gchar *device = NULL;
...
/**
 * mcm_calibrate_argyll_get_paper_size:
 **/
static GtkPaperSize *
mcm_calibrate_argyll_get_paper_size (McmCalibrate *calibrate, GtkWindow *window)
{
...
/**
 * mcm_calibrate_argyll_printer_convert_jpeg:
 **/
static gboolean
mcm_calibrate_argyll_printer_convert_jpeg (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	GDir *dir;
	const gchar *filename;
	gchar *filename_tiff;
...
/**
 * mcm_calibrate_argyll_printer:
 **/
static gboolean
mcm_calibrate_argyll_printer (McmCalibrate *calibrate, GtkWindow *window, GError **error)
{
	gboolean ret;
	gchar *cmdline = NULL;
	gchar *filename = NULL;
	...
	/* step 1 */
	if (print_kind == MCM_CALIBRATE_PRINT_KIND_LOCAL ||
	    print_kind == MCM_CALIBRATE_PRINT_KIND_GENERATE) {
		ret = mcm_calibrate_argyll_display_generate_patches (calibrate_argyll, error);
		if (!ret)
			goto out;
	}
	...
	/* print */
	if (print_kind == MCM_CALIBRATE_PRINT_KIND_LOCAL) {
		window = mcm_calibrate_dialog_get_window (priv->calibrate_dialog);
		ret = mcm_print_with_render_callback (priv->print, window, (McmPrintRenderCb) mcm_calibrate_argyll_render_cb, calibrate, error);
		if (!ret)
			goto out;
	}

	/* page setup, and then we're done */
	if (print_kind == MCM_CALIBRATE_PRINT_KIND_GENERATE) {

		/* get the paper size */
		paper_size = mcm_calibrate_argyll_get_paper_size (calibrate, window);
		width = gtk_paper_size_get_width (paper_size, GTK_UNIT_MM);
		height = gtk_paper_size_get_height (paper_size, GTK_UNIT_MM);
		...
		/* convert to jpegs */
		ret = mcm_calibrate_argyll_printer_convert_jpeg (MCM_CALIBRATE_ARGYLL (calibrate), error);
		if (!ret)
			goto out;

		cmdline = g_strdup_printf ("caja \"%s\"", working_path);
	...
	if (print_kind == MCM_CALIBRATE_PRINT_KIND_LOCAL) {
		...
		mcm_calibrate_dialog_set_show_expander (priv->calibrate_dialog, FALSE);
		mcm_calibrate_dialog_set_image_filename (priv->calibrate_dialog, "clock.svg");
	...
	if (print_kind == MCM_CALIBRATE_PRINT_KIND_ANALYZE) {
		...
		ret = mcm_calibrate_argyll_set_device_from_ti2 (calibrate, filename, error);
		if (!ret)
			goto out;
	}

	/* step 3 */
	ret = mcm_calibrate_argyll_display_read_chart (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 4 */
	ret = mcm_calibrate_argyll_device_generate_profile (calibrate_argyll, error);
	if (!ret)
		goto out;
...
/**
 * mcm_calibrate_argyll_pixbuf_remove_alpha:
 **/
static GdkPixbuf *
mcm_calibrate_argyll_pixbuf_remove_alpha (const GdkPixbuf *pixbuf)
{
...
/**
 * mcm_calibrate_argyll_check_and_remove_alpha:
 **/
static gboolean
mcm_calibrate_argyll_check_and_remove_alpha (McmCalibrateArgyll *calibrate_argyll, GError **error)
{
	gboolean ret = TRUE;
	...
	/* push new messages into the UI */
	mcm_calibrate_dialog_show (priv->calibrate_dialog, MCM_CALIBRATE_DIALOG_TAB_GENERIC, title, string->str);
	mcm_calibrate_dialog_set_show_button_ok (priv->calibrate_dialog, TRUE);
	mcm_calibrate_dialog_set_show_expander (priv->calibrate_dialog, FALSE);
	/* TRANSLATORS: button text to convert the RGBA image into RGB */
	mcm_calibrate_dialog_set_button_ok_id (priv->calibrate_dialog, _("Convert"));
	response = mcm_calibrate_dialog_run (priv->calibrate_dialog);
	if (response != GTK_RESPONSE_OK) {
		mcm_calibrate_dialog_hide (priv->calibrate_dialog);
		g_set_error_literal (error,
	...
	pixbuf_new = mcm_calibrate_argyll_pixbuf_remove_alpha (pixbuf);
	if (pixbuf_new == NULL) {
		g_set_error_literal (error,
				     MCM_CALIBRATE_ERROR,
				     MCM_CALIBRATE_ERROR_INTERNAL,
...
/**
 * mcm_calibrate_argyll_device:
 **/
static gboolean
mcm_calibrate_argyll_device (McmCalibrate *calibrate, GtkWindow *window, GError **error)
{
	McmCalibrateArgyll *calibrate_argyll = MCM_CALIBRATE_ARGYLL(calibrate);
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	gboolean ret;
	...
	/* step 1 */
	ret = mcm_calibrate_argyll_device_copy (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 1.5 */
	ret = mcm_calibrate_argyll_check_and_remove_alpha (calibrate_argyll, error);
	if (!ret)
		goto out;

	/* step 2 */
	ret = mcm_calibrate_argyll_device_measure (calibrate_argyll, error);
	if (!ret)
		goto out;
 ...
 **/
static void
mcm_calibrate_argyll_exit_cb (VteTerminal *terminal, McmCalibrateArgyll *calibrate_argyll)
{
	gint exit_status;
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
...
/**
 * mcm_calibrate_argyll_timeout_cb:
 **/
static gboolean
mcm_calibrate_argyll_timeout_cb (McmCalibrateArgyll *calibrate_argyll)
{
...
/**
 * mcm_calibrate_argyll_interaction_attach:
 **/
static void
mcm_calibrate_argyll_interaction_attach (McmCalibrateArgyll *calibrate_argyll)
{
	const gchar *title;
	const gchar *message;
	const gchar *filename;
	...
	filename = mcm_calibrate_argyll_get_colorimeter_image_attach (calibrate_argyll);
	...
	mcm_calibrate_dialog_show (priv->calibrate_dialog, MCM_CALIBRATE_DIALOG_TAB_GENERIC, title, message);
	mcm_calibrate_dialog_set_show_button_ok (priv->calibrate_dialog, TRUE);
	mcm_calibrate_dialog_set_image_filename (priv->calibrate_dialog, filename);
	mcm_calibrate_dialog_set_show_expander (priv->calibrate_dialog, FALSE);

	/* TRANSLATORS: button text */
	mcm_calibrate_dialog_set_button_ok_id (priv->calibrate_dialog, _("Continue"));
...
/**
 * mcm_calibrate_argyll_interaction_calibrate:
 **/
static void
mcm_calibrate_argyll_interaction_calibrate (McmCalibrateArgyll *calibrate_argyll)
{
	const gchar *title;
	const gchar *message;
	const gchar *filename;
	...
	/* get the image, if we have one */
	filename = mcm_calibrate_argyll_get_colorimeter_image_calibrate (calibrate_argyll);
...
/**
 * mcm_calibrate_argyll_interaction_surface:
 **/
static void
mcm_calibrate_argyll_interaction_surface (McmCalibrateArgyll *calibrate_argyll)
{
	const gchar *title;
	const gchar *message;
	const gchar *filename;
	...
	/* get the image, if we have one */
	filename = mcm_calibrate_argyll_get_colorimeter_image_screen (calibrate_argyll);
...
/**
 * mcm_calibrate_argyll_process_output_cmd:
 *
 * Return value: if FALSE then abort processing input
 **/
static gboolean
mcm_calibrate_argyll_process_output_cmd (McmCalibrateArgyll *calibrate_argyll, const gchar *line)
{
	const gchar *title;
	...
	if (g_strcmp0 (line, "Place instrument on test window.") == 0) {
		...
		mcm_calibrate_argyll_interaction_attach (calibrate_argyll);
		ret = FALSE;
		goto out;
	}
	...
	if (g_strcmp0 (line, "Set instrument sensor to calibration position,") == 0) {
		...
		mcm_calibrate_argyll_interaction_calibrate (calibrate_argyll);
		ret = FALSE;
		goto out;
	}
	...
	if (g_strcmp0 (line, "(Sensor should be in surface position)") == 0) {
		...
		mcm_calibrate_argyll_interaction_surface (calibrate_argyll);
		ret = FALSE;
		goto out;
	}
	...
	if (g_strstr_len (line, -1, "Measurement misread") != NULL) {
		...
		/* TRANSLATORS: button text */
		mcm_calibrate_dialog_set_button_ok_id (priv->calibrate_dialog, _("Try again"));
	...
	if (found != NULL) {
		...
		/* set state */
		priv->state = MCM_CALIBRATE_ARGYLL_STATE_WAITING_FOR_LOOP;
	...
	if (found != NULL) {
		mcm_calibrate_dialog_set_image_filename (priv->calibrate_dialog, "scan-target-good.svg");
	...
	if (g_str_has_prefix (line, "Strip read failed due to misread")) {
		...
		mcm_calibrate_dialog_set_show_button_ok (priv->calibrate_dialog, TRUE);
		mcm_calibrate_dialog_set_show_expander (priv->calibrate_dialog, TRUE);
		mcm_calibrate_dialog_set_image_filename (priv->calibrate_dialog, "scan-target-bad.svg");
	...
	if (g_str_has_prefix (line, "(Warning) Seem to have read strip pass ")) {
		...
		/* push new messages into the UI */
		mcm_calibrate_dialog_show (priv->calibrate_dialog, MCM_CALIBRATE_DIALOG_TAB_GENERIC, title_str, string->str);
		mcm_calibrate_dialog_set_show_button_ok (priv->calibrate_dialog, TRUE);
		mcm_calibrate_dialog_set_show_expander (priv->calibrate_dialog, TRUE);
		mcm_calibrate_dialog_set_image_filename (priv->calibrate_dialog, "scan-target-bad.svg");

		/* TRANSLATORS: button */
		mcm_calibrate_dialog_set_button_ok_id (priv->calibrate_dialog, _("Use anyway"));
	...
	if (g_str_has_prefix (line, "Ready to read strip pass ")) {
		...
		mcm_calibrate_dialog_set_show_button_ok (priv->calibrate_dialog, FALSE);
		mcm_calibrate_dialog_set_show_expander (priv->calibrate_dialog, TRUE);
		mcm_calibrate_dialog_set_image_filename (priv->calibrate_dialog, "scan-target.svg");
		goto out;
	}
 ...
 **/
static gboolean
mcm_calibrate_argyll_selection_func_cb (VteTerminal *terminal, glong column, glong row, gpointer data)
{
...
/**
 * mcm_calibrate_argyll_cursor_moved_cb:
 **/
static void
mcm_calibrate_argyll_cursor_moved_cb (VteTerminal *terminal, McmCalibrateArgyll *calibrate_argyll)
{
	gchar *output;
	gchar **split;
	guint i;
	...
	for (i=0; split[i] != NULL; i++) {
		...
		if (split[i][0] == '\0')
			continue;
		ret = mcm_calibrate_argyll_process_output_cmd (calibrate_argyll, split[i]);
		if (!ret)
			break;
	}
...
/**
 * mcm_calibrate_argyll_response_cb:
 **/
static void
mcm_calibrate_argyll_response_cb (GtkWidget *widget, GtkResponseType response, McmCalibrateArgyll *calibrate_argyll)
{
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	...
	if (response == GTK_RESPONSE_OK) {
		...
		if (priv->state == MCM_CALIBRATE_ARGYLL_STATE_WAITING_FOR_LOOP) {
			g_main_loop_quit (priv->loop);
			priv->state = MCM_CALIBRATE_ARGYLL_STATE_RUNNING;
		}
...
/**
 * mcm_calibrate_argyll_status_changed_cb:
 **/
static void
mcm_calibrate_argyll_status_changed_cb (McmPrint *print, GtkPrintStatus status, McmCalibrateArgyll *calibrate_argyll)
{
	const gchar *title;
...
/**
 * mcm_calibrate_argyll_class_init:
 **/
static void
mcm_calibrate_argyll_class_init (McmCalibrateArgyllClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	McmCalibrateClass *parent_class = MCM_CALIBRATE_CLASS (klass);
	object_class->finalize = mcm_calibrate_argyll_finalize;

	/* setup klass links */
	parent_class->calibrate_display = mcm_calibrate_argyll_display;
	parent_class->calibrate_device = mcm_calibrate_argyll_device;
	parent_class->calibrate_printer = mcm_calibrate_argyll_printer;
	parent_class->calibrate_spotread = mcm_calibrate_argyll_spotread;

	g_type_class_add_private (klass, sizeof (McmCalibrateArgyllPrivate));
}

/**
 * mcm_calibrate_argyll_init:
 **/
static void
mcm_calibrate_argyll_init (McmCalibrateArgyll *calibrate_argyll)
{
	calibrate_argyll->priv = MCM_CALIBRATE_ARGYLL_GET_PRIVATE (calibrate_argyll);
	...
	calibrate_argyll->priv->state = MCM_CALIBRATE_ARGYLL_STATE_IDLE;
	calibrate_argyll->priv->print = mcm_print_new ();
	g_signal_connect (calibrate_argyll->priv->print, "status-changed",
			  G_CALLBACK (mcm_calibrate_argyll_status_changed_cb), calibrate_argyll);
	calibrate_argyll->priv->calibrate_dialog = mcm_calibrate_dialog_new ();
	g_signal_connect (calibrate_argyll->priv->calibrate_dialog, "response",
			  G_CALLBACK (mcm_calibrate_argyll_response_cb), calibrate_argyll);
	...
	calibrate_argyll->priv->terminal_child_exited_id =
		g_signal_connect (calibrate_argyll->priv->terminal, "child-exited",
				G_CALLBACK (mcm_calibrate_argyll_exit_cb), calibrate_argyll);
	calibrate_argyll->priv->terminal_cursor_moved_id =
		g_signal_connect (calibrate_argyll->priv->terminal, "cursor-moved",
				G_CALLBACK (mcm_calibrate_argyll_cursor_moved_cb), calibrate_argyll);
	mcm_calibrate_dialog_pack_details (calibrate_argyll->priv->calibrate_dialog,
					   calibrate_argyll->priv->terminal);
}

/**
 * mcm_calibrate_argyll_finalize:
 **/
static void
mcm_calibrate_argyll_finalize (GObject *object)
{
	McmCalibrateArgyll *calibrate_argyll = MCM_CALIBRATE_ARGYLL (object);
	McmCalibrateArgyllPrivate *priv = calibrate_argyll->priv;
	...
	G_OBJECT_CLASS (mcm_calibrate_argyll_parent_class)->finalize (object);
}

/**
 * mcm_calibrate_argyll_new:
 *
 * Return value: a new McmCalibrateArgyll object.
 **/
McmCalibrateArgyll *
mcm_calibrate_argyll_new (void)
{
	McmCalibrateArgyll *calibrate_argyll;
	calibrate_argyll = g_object_new (MCM_TYPE_CALIBRATE_ARGYLL, NULL);
	return MCM_CALIBRATE_ARGYLL (calibrate_argyll);
}
