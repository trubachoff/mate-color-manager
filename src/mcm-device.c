...
/**
 * mcm_device_get_profile_filenames:
 *
 * Return value: the filenames. Do not free this value
 **/
gchar **
mcm_device_get_profile_filenames (McmDevice *device)
{
	g_return_val_if_fail (MCM_IS_DEVICE (device), NULL);
...
/**
 * mcm_device_set_profile_filenames:
 **/
void
mcm_device_set_profile_filenames (McmDevice *device, gchar **profile_filenames)
{
	g_return_if_fail (MCM_IS_DEVICE (device));
...
mcm_device_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_TITLE:
		...
		break;
	case PROP_PROFILE_FILENAME:
		mcm_device_set_profile_filenames (device, (gchar**)g_value_get_boxed (value));
		break;
	case PROP_GAMMA:
