...
/**
 * mcm_dump_edid_filename:
 **/
static gboolean
mcm_dump_edid_filename (const gchar *filename)
{
	...
	/* print data */
	monitor_name = mcm_edid_get_monitor_name (edid);
	if (monitor_name != NULL) {
		/* TRANSLATORS: this is debugging output for the supplied EDID file */
		g_print ("  %s: %s\n", _("Monitor name"), monitor_name);
	}
	vendor_name = mcm_edid_get_vendor_name (edid);
	if (vendor_name != NULL) {
		/* TRANSLATORS: this is debugging output for the supplied EDID file */
		g_print ("  %s: %s\n", _("Vendor name"), vendor_name);
	}
	serial_number = mcm_edid_get_serial_number (edid);
	...
	pnp_id = mcm_edid_get_pnp_id (edid);
	if (pnp_id != NULL) {
		/* TRANSLATORS: this is debugging output for the supplied EDID file */
		g_print ("  %s: %s\n", _("PNP identifier"), pnp_id);
	}
	width = mcm_edid_get_width (edid);
	height = mcm_edid_get_height (edid);
	if (width != 0) {
		/* TRANSLATORS: this is debugging output for the supplied EDID file */
		g_print ("  %s: %ix%i\n", _("Size"), width, height);
	}
	gamma = mcm_edid_get_gamma (edid);
	if (gamma > 0.0f) {
		/* TRANSLATORS: this is debugging output for the supplied EDID file */
		g_print ("  %s: %f\n", _("Gamma"), gamma);
	}
...
main (int argc, char **argv)
{
	...
	context = g_option_context_new ("mate-color-manager dump edid program");
	...
	if (files != NULL) {
		for (i=0; files[i] != NULL; i++) {
			...
			g_print ("%s: %s\n", _("EDID dump"), files[i]);
			mcm_dump_edid_filename (files[i]);
		}
		goto out;
	}
	...
	for (i=0; outputs[i] != NULL; i++) {
		...
		ret = mate_rr_output_is_connected (outputs[i]);
		if (!ret)
			continue;

		/* get data */
		data = mate_rr_output_get_edid_data (outputs[i]);
		if (data == NULL)
			continue;

		/* get output name */
		output_name = g_strdup (mate_rr_output_get_name (outputs[i]));
		mcm_utils_alphanum_lcase (output_name);
		...
		if (ret) {
			...
			g_print ("\n");
			mcm_dump_edid_filename (filename);
		} else {
