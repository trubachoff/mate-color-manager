...
void
mcm_edid_test_parse_edid_file (EggTest *test, McmEdid *edid, const gchar *datafile, McmEdidTestData *test_data)
{
	gchar *filename;
	const gchar *monitor_name;
	const gchar *vendor_name;
	...
	ret = mcm_edid_parse (edid, data, &error);
	if (ret)
		egg_test_success (test, NULL);
	else
...
mcm_edid_test (EggTest *test)
{
	...
	if (!egg_test_start (test, "McmEdid"))
		return;
	...
	mcm_edid_test_parse_edid_file (test, edid, "LG-L225W-External.bin", &test_data);
	...
	test_data.width = 33;
	test_data.gamma = 2.2f;
	mcm_edid_test_parse_edid_file (test, edid, "Lenovo-T61-Internal.bin", &test_data);
