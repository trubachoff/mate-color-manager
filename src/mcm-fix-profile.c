...
/*
 * mcm_fix_profile_filename:
 */
static gboolean
mcm_fix_profile_filename (const gchar *filename, const gchar *description, const gchar *copyright, const gchar *model, const gchar *manufacturer)
{
	gboolean ret = TRUE;
...
/*
 * mcm_fix_profile_lcms_error_cb:
 */
static int
mcm_fix_profile_lcms_error_cb (int ErrorCode, const char *ErrorText)
{
...
main (int argc, char **argv)
{
	...
	/* setup LCMS */
	cmsSetErrorHandler (mcm_fix_profile_lcms_error_cb);
	cmsErrorAction (LCMS_ERROR_SHOW);
	cmsSetLanguage ("en", "US");

	/* fix each profile */
	for (i=0; files[i] != NULL; i++) {
		ret = mcm_fix_profile_filename (files[i], description, copyright, model, manufacturer);
		if (!ret) {
			retval = 1;
			goto out;
		}
