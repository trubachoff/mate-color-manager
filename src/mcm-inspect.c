...
/**
 * mcm_inspect_get_properties_collect_cb:
 **/
static void
mcm_inspect_get_properties_collect_cb (const char *key, const GValue *value, gpointer user_data)
{
...
mcm_inspect_get_properties (void)
{
	...
	ret = dbus_g_proxy_call (proxy, "GetAll", &error,
				 G_TYPE_STRING, "org.mate.ColorManager",
				 G_TYPE_INVALID,
	...
	/* process results */
	if (hash != NULL)
		g_hash_table_foreach (hash, (GHFunc) mcm_inspect_get_properties_collect_cb, NULL);
out:
	if (hash != NULL)
