...
void
mcm_profile_test_parse_file (EggTest *test, const guint8 *datafile, McmProfileTestData *test_data)
{
	gchar *filename = NULL;
	const gchar *copyright;
	const gchar *manufacturer;
...
void
mcm_profile_test (EggTest *test)
{
	McmProfileTestData test_data;

	if (!egg_test_start (test, "McmProfile"))
		return;
	...
	mcm_profile_test_parse_file (test, "bluish.icc", &test_data);
	...
	mcm_profile_test_parse_file (test, "AdobeGammaTest.icm", &test_data);
