...
static gboolean mcm_trc_widget_expose (GtkWidget *trc, GdkEventExpose *event);
...
mcm_trc_widget_class_init (McmTrcWidgetClass *class)
{
	...
	widget_class->expose_event = mcm_trc_widget_expose;
...
/**
 * mcm_trc_widget_expose:
 *
 * Just repaint the entire trc widget on expose.
 **/
static gboolean
mcm_trc_widget_expose (GtkWidget *trc, GdkEventExpose *event)
{
	cairo_t *cr;
...
void
mcm_trc_widget_test (EggTest *test)
{
	GtkWidget *widget;
	GtkWidget *image;
	GtkWidget *dialog;
	...
	if (!egg_test_start (test, "McmTrcWidget"))
		return;
